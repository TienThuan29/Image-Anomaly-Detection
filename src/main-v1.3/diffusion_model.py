...
class UNetModel(nn.Module):
    """
    UNet model modified for direct image prediction instead of noise prediction.
    Key changes:
    1. Output activation function (Tanh or Sigmoid)
    2. Different initialization strategy
    3. Modified output layer design
    4. Optional residual connection to input
    """
    ...
    def __init__(
        ...
        # Modified output layer for image prediction
        self.out = nn.Sequential(
            GroupNorm32(32, ch),
            nn.SiLU(),
            nn.Conv2d(base_channels * channel_mults[0], self.out_channels, 3, padding=1)
            # Note: Removed zero_module for better initialization
        )

        # Initialize output layer with smaller weights for stable training
        nn.init.xavier_uniform_(self.out[-1].weight, gain=0.1)
    ...
    def forward(self, x, time):
        """
        Forward pass that predicts the clean image directly.
        ...
        Returns:
            pred_image: Predicted clean image
        """
        ...
        pred_image = self.output_activation_fn(pred_image)

        # Scale output if needed
        if self.output_scale != 1.0:
            pred_image = pred_image * self.output_scale

        # Optional residual connection to input
        if self.use_input_residual:
            # Weighted residual connection (can help with training stability)
            alpha = 0.1  # Small weight for residual
            pred_image = pred_image + alpha * input_image
...
def safe_group_norm(num_channels, max_groups=32):
    for g in reversed(range(1, max_groups + 1)):
        if num_channels % g == 0:
            return nn.GroupNorm(g, num_channels)
    raise ValueError(f"Cannot find valid group for {num_channels}")