...
def compute_anomaly_map_advanced(x, x_recon, feature_extractor=None):
    """
    Advanced anomaly map computation using multiple metrics:
    - L1 pixel difference
    - L2 pixel difference  
    - SSIM-based difference
    - Perceptual difference (if feature_extractor provided)
    - Combined map
    """
    results = {}
...
def get_anomaly_maps(use_perceptual: bool = True):
    set_seed(_seed)
    ...
    if use_perceptual:
        visualization_results['anomaly_maps']['perceptual'] = []
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(test_loader, desc="run inference ")):
            ...
            # Compute advanced anomaly maps
            anomaly_maps_dict = compute_anomaly_map_advanced(
                x=images,
            ...
            for map_type, maps in anomaly_maps_dict.items():
                all_anomaly_maps[map_type].extend([am.cpu() for am in maps])
            ...
            if batch_count < _max_visualization_batches:
                ...
                for map_type, maps in anomaly_maps_dict.items():
                    visualization_results['anomaly_maps'][map_type].append(maps[:max_images])
...
def run_inference(all_labels, all_anomaly_maps, all_gt_masks, use_perceptual: bool = True):
    results = []
    score_types = ['max', 'mean', 'std']

    # Evaluate each anomaly map type
    for map_type, anomaly_maps in all_anomaly_maps.items():
        if not anomaly_maps:  # Skip if no maps for this type
            continue
            
        print(f'\n========== Evaluating {map_type.upper()} anomaly maps ==========')
        
        pixel_auroc = eval_auroc_pixel(anomaly_maps, all_gt_masks)
        for score_type in score_types:
            print(f'-- Type: {score_type} --')
            image_scores = calc_image_score(anomaly_maps, score_type)
            image_auroc = eval_auroc_image(all_labels, image_scores.cpu().numpy().tolist())

            evaluation_result = {
                'category': _testing_category,
                'method': f'advanced_{map_type}',
                ...
                'feature_extractor': 'VGG16' if use_perceptual and map_type == 'perceptual' else None,
                'anomaly_map_weights': {
                    'l1_weight': 0.4,
                    'l2_weight': 0.3,
                    'ssim_weight': 0.2,
...
def run_all_inference():
    ...
    # Test with perceptual features
    print('========== Advanced Anomaly Detection with Perceptual Features ===========')
    testing_result_dir = config.testing.test_result_base_dir + _testing_category + '/advanced_perceptual/'
    ...
    # Test without perceptual features
    # print('\n========== Advanced Anomaly Detection without Perceptual Features ===========')
    # testing_result_dir = config.testing.test_result_base_dir + _testing_category + '/advanced_no_perceptual/'
    # os.makedirs(testing_result_dir, exist_ok=True)
    
    # all_labels, all_anomaly_maps, all_gt_masks, visualization_results = get_anomaly_maps(use_perceptual=False)
    # no_perceptual_results = run_inference(all_labels, all_anomaly_maps, all_gt_masks, use_perceptual=False)
    # all_results.extend(no_perceptual_results)
    ...
    # results_path = os.path.join(testing_result_dir, f'inference_results.json')
    # with open(results_path, 'w') as f:
    #     json.dump(no_perceptual_results, f, indent=2)
